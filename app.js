...
async function login(){
    ...
    openMemberDashboard({...data.user,name:profile?.full_name||data.user.user_metadata?.full_name||data.user.email?.split('@')[0]||'Member',email:data.user.email||'',phone:profile?.phone||'',house_no:profile?.house_number||'',address:profile?.address||'',role:profile?.role||'member'});
