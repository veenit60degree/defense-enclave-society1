...
<style id="public-section-white-layouts">
#publicEventsSection,
#publicGallerySection,
#publicSocietyMapSection,
...
function applyPublicSectionWhiteLayouts(){
    const selectors = [
        '#publicComplaintsSection',
        '#publicEventsSection',
        '#publicGallerySection',
    ...
    // Apply the same visual treatment used by Society Work:
    // white card, rounded corners, subtle shadow, spacing and responsive sizing.
    selectors.forEach(sel=>{
        const section=document.querySelector(sel);
        if(!section)return;
        section.style.background='#fff';
        section.style.borderRadius='18px';
        section.style.boxShadow='0 4px 18px rgba(16,24,40,.06)';
...
async function renderPublic(){
    ...
    const eventsSection=eventGrid?.closest('section') || publicSectionByHeading(['Events']);
    if(eventsSection) eventsSection.id='publicEventsSection';
    if(eventGrid){
        if(events.error){
    ...
    const mapSection=mapDiv?.closest('section') || publicSectionByHeading(['Society Map','Map']);
    if(mapSection) mapSection.id='publicSocietyMapSection';
    if(mapDiv){
    ...
    if(gallerySection) gallerySection.id='publicGallerySection';
    if(gallerySection){
    ...
    applyPublicSectionWhiteLayouts();
}
