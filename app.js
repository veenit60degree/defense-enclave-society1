...
async function login(){
    ...
    if(!sb){
        return toast('Please add your Supabase Publishable key in app.js');
    }
...
async function register(){
    ...
    if(!name || !email || !password || !house_no){
        return toast('Please fill name, email, house and password');
    }
    ...
    if(!data.user){
        return toast('Registration could not be completed');
    }

    /*
     * A database trigger may create the profiles row.
    ...
    if(profileResult.error){
        ...
        return toast('Account created, but profile details could not be saved');
    }
...
document.getElementById('memberTour').onclick=()=>{
    toast('Tour: dashboard → profile → complaints → work → events → gallery');
};

document.querySelectorAll('#memberApp .nav-item').forEach(button=>{
    button.addEventListener('click',()=>{
        const page=button.dataset.p;
        ...
        memberPage(page,user);
    });
});

memberPage('dash',user)}
