...
async function saveFinance(){
    ...
    await adminPage('dashboard',window.__adminUser);
}
...
async function adminPage(p,user){
 ...
 if(p==='dashboard'){
    if(!sb){
        c.innerHTML=`<div class="panel"><h3>Admin Dashboard</h3><div class="muted">Database connection is not configured.</div></div>`;
        return;
    }

    const {data:finance,error}=await sb.from('society_finance')
        .select('society_fund,total_expenses,active_maintenance,pending_tasks')
        .eq('id',1).maybeSingle();

    if(error){
        console.error('Admin Dashboard finance load error:',error);
        c.innerHTML=`<div class="panel"><h3>Admin Dashboard</h3><div class="muted">Unable to load saved database data.</div><p>${error.message}</p></div>`;
        return;
    }

    const savedFinance=finance||{
        society_fund:0,
        total_expenses:0,
    ...
    window.__societyFinance=savedFinance;

    c.innerHTML=`<div class="hero">
        <div><div class="eyebrow">ADMINISTRATION</div><h2>Welcome, ${user.name||'Admin'}</h2>
        <div class="muted">All values below are loaded from saved database data.</div></div>
    </div>
    <div class="stats">
        <div class="stat"><div class="stat-head">Society Fund<span>●</span></div><div class="value">₹${Number(savedFinance.society_fund||0).toLocaleString('en-IN')}</div><div class="trend">Saved database value</div></div>
        <div class="stat"><div class="stat-head">Total Expenses<span>●</span></div><div class="value">₹${Number(savedFinance.total_expenses||0).toLocaleString('en-IN')}</div><div class="trend">Saved database value</div></div>
        <div class="stat"><div class="stat-head">Active Maintenance<span>●</span></div><div class="value">₹${Number(savedFinance.active_maintenance||0).toLocaleString('en-IN')}</div><div class="trend">Saved database value</div></div>
        <div class="stat"><div class="stat-head">Pending Tasks<span>●</span></div><div class="value">${Number(savedFinance.pending_tasks||0)}</div><div class="trend">Saved database value</div></div>
    </div>
    <div class="grid-2-equal">
        <div class="panel"><h3>Management</h3><div class="activity">
            <div class="activity-item"><div class="activity-icon">₹</div><div><strong>Society Finance</strong><p>Manage saved financial information.</p></div></div>
            <div class="activity-item"><div class="activity-icon">♙</div><div><strong>Members</strong><p>Manage saved member profiles.</p></div></div>
            <div class="activity-item"><div class="activity-icon">▧</div><div><strong>Gallery</strong><p>Manage saved gallery photos.</p></div></div>
        </div></div>
    ...
    let finance={society_fund:0,total_expenses:0,active_maintenance:0,pending_tasks:0};
...
async function restoreLoginSession(){
    ...
    try{
        ...
        if(user.role==='admin') await openAdminDashboard(user);
        else await openMemberDashboard(user);
    }catch(error){
