 ...
 if(p==='dashboard'){
    ...
    if(sb){
        const {data,error}=await sb
            .from('society_finance')
            .select('society_fund,total_expenses,active_maintenance,pending_tasks')
            .eq('id',1)
            .maybeSingle();

        if(error){
            console.error('Admin Dashboard finance error:',error);
            toast('Unable to load finance: '+error.message);
        }else if(data){
            finance={
    ...
    <div class="stats">
        <div class="stat"><div class="stat-head">Society Fund<span>●</span></div><div class="value">₹${finance.society_fund.toLocaleString('en-IN')}</div><div class="trend">Manage in Finance</div></div>
        <div class="stat"><div class="stat-head">Total Expenses<span>●</span></div><div class="value">₹${finance.total_expenses.toLocaleString('en-IN')}</div><div class="trend">Manage expenses</div></div>
        <div class="stat"><div class="stat-head">Active Maintenance<span>●</span></div><div class="value">₹${finance.active_maintenance.toLocaleString('en-IN')}</div><div class="trend">Manage maintenance</div></div>
        <div class="stat"><div class="stat-head">Pending Tasks<span>●</span></div><div class="value">${finance.pending_tasks}</div><div class="trend">Review society work</div></div>
    </div>
    <div class="grid-2-equal"><div class="panel"><h3>Management</h3><div class="activity"><div class="activity-item"><div class="activity-icon">₹</div><div><strong>Society Finance</strong><p>Update fund, expenses and maintenance.</p></div></div><div class="activity-item"><div class="activity-icon">♙</div><div><strong>Members</strong><p>Add, edit and manage society members.</p></div></div><div class="activity-item"><div class="activity-icon">▧</div><div><strong>Gallery</strong><p>Manage photo categories and uploads.</p></div></div></div></div>
    <div class="panel"><h3>Quick actions</h3><div class="form-grid"><button class="primary-btn" onclick="adminPage('finance',window.__adminUser)">Manage Finance</button><button class="primary-btn" onclick="adminPage('members',window.__adminUser)">Manage Members</button><button class="primary-btn" onclick="adminPage('events',window.__adminUser)">Manage Events</button><button class="primary-btn" onclick="adminPage('gallery',window.__adminUser)">Manage Gallery</button></div></div></div>`;
}else if(p==='finance'){
